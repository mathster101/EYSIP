...
def build_model():# builds the model and returns it
   ...
   model.add(Embedding(MAX_NB_WORDS, EMBEDDING_DIM, input_length=50))#embed layer
   ...
   try:
      model.load_weights(PATH + "weights_bidir_oversample.best.hdf5")#check if pre trained exists
   except:
...
def train_it(PATH,WEIGHT_PATH): #train model 
   ...
   checkpoint = ModelCheckpoint(PATH + "weights_bidir_oversample.best.hdf5", monitor='val_accuracy', verbose=1, save_best_only=True, mode='max') #checkpointing it
